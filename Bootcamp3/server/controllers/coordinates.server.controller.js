  ...
  if(req.body.address) {
    ...
    request({
      ...
      qs: options
        ...
        rawCoordinates = JSON.parse(body);
        ...
        */
        if (!error || response.status.code == 200 || body.results.length > 0) {
          req.results = {
            lat: rawCoordinates.results[0].geometry.lat,
            lng: rawCoordinates.results[0].geometry.lng
          }
        }
        next();
    });
